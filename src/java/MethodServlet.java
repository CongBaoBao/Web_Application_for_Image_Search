...
public class MethodServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(methodName.equals("search")){
            ...
            splitedTags = Translate.execute(splitedTags,Language.ENGLISH);
	    ...
	    searchParams.setTags(splitedTags);
...
//To change body of generated methods, choose Tools | Templates.
    ...
    private String[] splitedTags(String otherTranslation) {
         //To change body of generated methods, choose Tools | Templates.
        char[] cache = otherTranslation.toCharArray();
        
        for(int i=0; i< cache.length; i++){
            if(cache[i]==','||cache[i]=='.'||cache[i]=='!'||cache[i]=='?'||cache[i]==';'||cache[i]==':'||cache[i]=='('||cache[i]==')'||cache[i]=='['||cache[i]==']'||cache[i]=='{'||cache[i]=='}'){
                cache[i]=' ';
            }
        }
        String[] result = new String(cache).trim().replaceAll( " + ", " ").split(" ");
        return result;
    }
